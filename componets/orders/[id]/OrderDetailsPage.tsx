...
import Link from "next/link";
import useOrders from "@/customHooks/order/useOrders";
import { resolveOrderStatus } from "../orderConstants";
import {
  ArrowLeft,
  Clock,
  MapPin,
...
const StatusTimeline = ({
  ...
  const steps = [
    ...
    },
    {
      label: "Ready for Pickup",
      icon: Package,
      time: status === "ready" ? "Ready" : null,
    },
    {
      label: "Picked Up",
      icon: Bike,
      time: status === "picked_up" ? "Picked up" : null,
    },
    {
      label: "Out for Delivery",
      icon: Bike,
  ...
  const currentStepIndex = steps.findIndex((_, index) => {
    if (status === "delivered") return index === 6;
    if (status === "out_for_delivery") return index === 5;
    if (status === "picked_up") return index === 4;
...
export default function OrderDetailsPage() {
  ...
  const displayStatus = resolveOrderStatus(selectedOrder);
  const status = STATUS_CONFIG[displayStatus] || STATUS_CONFIG.placed;
  const StatusIcon = status.icon;
  const canCancel = CANCELLABLE_STATUSES.includes(displayStatus);
