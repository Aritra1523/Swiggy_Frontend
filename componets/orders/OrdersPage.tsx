...
import OrderFilters from "./OrderFilters";
import OrderCard from "./OrderCard";
import { type StatusFilter, resolveOrderStatus } from "./orderConstants";
...
const [activeFilter, setActiveFilter] =
  ...
  useEffect(() => {
    ...
    // The delivery partner's actions (picked up / start delivery / delivered)
    // update `deliveryStatus`, which is broadcast on a separate event.
    const handleDeliveryStatus = (data: any) => {
      console.log("Delivery status update:", data);
      if (data.userId && data.userId !== userId) return;

      handleFetchMyOrders();
      const next = data.currentStatus ?? data.deliveryStatus;
      Swal.fire({
        toast: true,
        position: "top-end",
        icon: "info",
        title: `Delivery update: ${String(next).replaceAll("_", " ")}`,
        showConfirmButton: false,
        timer: 3000,
        timerProgressBar: true,
      });
    ...
    socket.on("connect", handleConnect);
    socket.on("order:status", handleOrderStatus);
    socket.on("order:delivery-status", handleDeliveryStatus);
    socket.on("delivery:status", handleDeliveryStatus);

    return () => {
      socket.off("connect", handleConnect);
      socket.off("order:status", handleOrderStatus);
      socket.off("order:delivery-status", handleDeliveryStatus);
  ...
  const filteredOrders = orders.filter((order) => {
    if (activeFilter !== "all" && resolveOrderStatus(order) !== activeFilter)
      return false;
    if (searchTerm.trim()) {
      const search = searchTerm.toLowerCase();
      const restaurantName = order.restaurant?.restaurantName?.toLowerCase() || "";
