...
import Link from "next/link";
import Image from "next/image";
import { useMyRestaurant, useFoodList, useRestaurantStatus } from "@/customHooks/owner/useFoodManagement";
import {
  Store,
  UtensilsCrossed,
  Plus,
...
export default function OwnerDashboardPage() {
  const { data: restaurantRes, isLoading: restaurantLoading, refetch: refetchRestaurant } = useMyRestaurant();
  const { data: foodsRes, isLoading: foodsLoading } = useFoodList(1, 100);
  const [selectedPeriod, setSelectedPeriod] = useState<"today" | "week" | "month">("week");
  ...
  const isOpen = localIsOpen !== null ? localIsOpen : (restaurant?.isOpen ?? false);
