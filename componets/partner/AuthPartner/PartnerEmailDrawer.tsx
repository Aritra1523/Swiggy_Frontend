...
export default function PartnerEmailDrawer({
  close,
  openOtp,
  accountEmail,
}: Props) {
  const { register, handleSubmit, errors, isSubmitting } = useApplyRestaurant(
    openOtp,
    accountEmail ?? "",
  );
    ...
    <div className="fixed inset-0 bg-black/50 z-50 backdrop-blur-sm" onClick={close}>
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-2 mb-6 text-sm sm:text-base">
            We'll send an OTP to your account email to start your restaurant
            application.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <input
              ...
              readOnly
              {...register("email")}
              className="w-full border-2 border-gray-100 bg-gray-50 rounded-lg p-3 outline-none text-gray-500 cursor-not-allowed"
            />
