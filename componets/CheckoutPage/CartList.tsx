...
// );
import Image from "next/image";
import { CartItem as CartItemType, CartRestaurant } from "@/typescript/order/order";
...
const CartItem = ({ item, restaurant, onRemove }: CartItemProps) => {
  const food = item.food;
  if (!food) return null;
...
export const CartList = ({ items, restaurant, onRemoveItem }: CartListProps) => (
  <div className="lg:col-span-2 space-y-4">
    {items.map((item) => (
      <CartItem
        key={item.food?._id}
